...
static const char *ROLE_NAME[] = {"FOLLOWER", "CANDIDATE", "LEADER"};
...
static void render_tui(void) {
  ...
  printf(C_BG_BLUE C_BOLD "  GRID NODE V9.0  |  ID: %-15s  |  ROLE: %-10s  |  "
                          "LEADER: %-15s  " C_RESET "\n",
         g_my_ip, ROLE_NAME[(int)g_role],
         g_leader_id ? (g_role == LEADER ? g_my_ip : "...") : "ELECTING");
  ...
  for (int i = 0; i < MAX_PEERS; i++) {
    ...
    printf("│ %-16s │ %-18s │ %7.1f%%   │ %-8s │\n", g_peers[i].ip, st,
           g_peers[i].cpu_avg,
           g_peers[i].id == g_leader_id ? C_YELLOW "YES" C_CYAN : "no");
  }
...
static void enqueue_job(int sender_fd, uint8_t *payload, uint32_t len,
  ...
  if (q_size >= MAX_QUEUE_SIZE) {
    send_msg(sender_fd, MSG_REJECTED, "Grid queue full", 15);
    return;
  }
  ...
  q_size++;
  grid_log(C_YELLOW, "[Queue] Workers busy, job queued. Queue size: %d",
           q_size);
  const char *msg = "\n[Grid]: All workers busy. Placed in queue...\n";
...
static void assign_job(Job *j) {
  ...
  if (w >= 0) {
    ...
    grid_log(C_MAGENTA, "[Balancer] Job %llu → worker %s",
             (unsigned long long)j->id, g_peers[w].ip);
    send_job_to_peer(w, j);
  } else {
    /* No peers: run locally */
    j->worker_id = g_my_id;
    grid_log(C_CYAN, "[Balancer] Job %llu → self (local exec)",
...
static void reassign_jobs_from(uint32_t dead_worker_id) {
  ...
  for (Job *j = g_jobs; j; j = j->next) {
    if (!j->active || j->worker_id != dead_worker_id)
      continue;
    grid_log(C_YELLOW,
             "[Failover] Job %llu orphaned by dead worker. Reassigning...",
             (unsigned long long)j->id);
    const char *msg =
...
static void start_election(void) {
  ...
  grid_log(C_YELLOW, "[Election] Starting election. My ID: %u", g_my_id);
...
static void become_leader(void) {
  ...
  grid_log(C_GREEN C_BOLD, "[Election] *** I AM THE NEW LEADER *** (%s)",
           g_my_ip);
...
static void *peer_loop_thread(void *arg) {
  ...
  Peer *p = &g_peers[pidx];
  grid_log(C_BLUE, "[Network] Peer connected: %s", p->ip);
  ...
  while (1) {
    ...
    case MSG_ELECTION_OK:
      ...
      g_role = FOLLOWER;
      pthread_mutex_unlock(&g_elec_mu);
      grid_log(C_YELLOW, "[Election] Yielding to higher node %s", p->ip);
      break;
    ...
    case MSG_JOB_ASSIGN: {
      ...
      grid_log(C_CYAN, "[Worker] Executing job %llu from leader",
    ...
    case MSG_WORKER_STATUS: {
      ...
      grid_log(C_YELLOW, "[Failover] Worker %s has live job %llu (type %d)",
  ...
  if (dead_id == g_leader_id && g_role != LEADER) {
    grid_log(C_YELLOW, "[Election] Leader died! Starting election...");
    start_election();
...
static void *dispatcher_loop_thread(void *arg) {
  ...
  grid_log(C_BLUE, "[Dispatcher] Sender connected from %s", ip);
  ...
  while (1) {
    ...
    if (hdr.type == MSG_EXEC_REQ || hdr.type == MSG_PROJECT_REQ) {
      ...
      if (!is_project && scan_for_malware(buf)) {
        ...
        grid_log(C_RED, "[Security] STRIKE from %s (%d/3)", ip, strikes);
  ...
  pthread_mutex_unlock(&g_disp_mu);
  close(fd);
  grid_log(C_YELLOW, "[Dispatcher] Sender %s disconnected.", ip);
  return NULL;
}
...
static void *monitor_thread(void *arg) {
  ...
  while (1) {
    ...
    if (g_role == FOLLOWER && g_leader_id != 0 && g_last_ldr_hb != 0 &&
      ...
      grid_log(C_YELLOW,
               "[Monitor] Leader heartbeat timeout. Starting election.");
      g_leader_id = 0;
      g_last_ldr_hb = 0;
      start_election();
    }
    ...
    for (int i = 0; i < MAX_PEERS; i++) {
      ...
      if (now - g_peers[i].last_hb > PEER_DEAD_SECS) {
        ...
        if (dead_id == g_leader_id && g_role != LEADER) {
          grid_log(C_YELLOW,
                   "[Monitor] That was the leader. Starting election.");
          start_election();
        } else if (g_role == LEADER) {
          reassign_jobs_from(dead_id);
        }
...
static void handle_sigint(int sig) {
  ...
  printf(C_GREEN "Node shut down. Ledger saved to grid_ledger.csv\n" C_RESET);
  exit(0);
}
...
int main(void) {
  ...
  sleep(2);
  if (g_leader_id == 0) {
    grid_log(C_YELLOW, "[Startup] No leader found. Starting election...");
    start_election();
  }
